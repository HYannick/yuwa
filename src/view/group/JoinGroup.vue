...
<template>
  <section class="bg-zinc-100 dark:bg-zinc-900 w-screen min-h-screen md:max-w-screen min-h-screen-xl md:m-auto p-5 flex flex-col items-center">
    <h3 class="text-5xl font-bold font-display my-10 text-center text-gray-800 dark:text-zinc-400">
      <span v-if="error">Uh oh..</span>
      <span v-else-if="success"><span class="font-normal mb-2 text-4xl text-gray-800 dark:text-zinc-400">Welcome to</span><br> <span
          class="font-bold text-gray-800 dark:text-zinc-400">{{ groupNameFromURL }}</span></span>
    </h3>
    <div class="flex-1 flex flex-col h-full justify-center gap-36">
      <template v-if="error">
        <template v-if="error.type === JoinErrorTypeEnum.ALREADY_JOINED">
          <p class="text-2xl text-center mb-20 text-gray-800 dark:text-zinc-400">Looks like you are already in this group! Start splitting :)</p>
        ...
        <template v-else-if="error.type === JoinErrorTypeEnum.EXPIRED_TOKEN || error.type == JoinErrorTypeEnum.INVALID_TOKEN">
          <p class="text-2xl text-center mb-20 text-gray-800 dark:text-zinc-400">Looks like the token is not valid or expired ! Ask again to your group manager :)</p>
        ...
        <template v-else-if="error.type === JoinErrorTypeEnum.MAX_USES_REACHED">
          <p class="text-2xl text-center mb-20 text-gray-800 dark:text-zinc-400">There is too many members in the group :(</p>
          <BaseButton @click="router.push({ name: 'home' })" type="button" class="w-full">Go to Home</BaseButton>
        </template>
        <template v-else>
          <p class="text-2xl text-center mb-20 text-gray-800 dark:text-zinc-400">We were unable to process the request :(. The link might be invalid or expired. Ask again to your
            group manager :)</p>
          <BaseButton @click="router.push({ name: 'home' })" type="button" class="w-full">Go to Home</BaseButton>
        </template>
      </template>
      <div v-else-if="success">
        <p class="text-2xl text-center mb-20 text-gray-800 dark:text-zinc-400">You have successfully joined the group!</p>
        <BaseButton @click="router.push({ path: `/groups/${groupId}` })" type="button" class="w-full">Continue</BaseButton>
      </div>
      <div v-else>
        <p class="text-2xl text-center mb-20 text-gray-800 dark:text-zinc-400">Attempt Joining group...</p>
      </div>
    </div>
  </section>
</template>
